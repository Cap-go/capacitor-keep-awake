...
const actions = [
  {
    id: 'check-supported',
    label: 'Check if supported',
    description: 'Calls isSupported() to determine if keep awake is supported on this platform.',
    inputs: [],
    run: async () => {
  ...
  },
  {
    id: 'keep-awake',
    label: 'Keep awake',
    description: 'Prevents the device screen from dimming or sleeping.',
    inputs: [],
    run: async () => {
  ...
  },
  {
    id: 'allow-sleep',
    label: 'Allow sleep',
    description: 'Allows the device screen to dim and sleep normally.',
    inputs: [],
    run: async () => {
      await plugin.allowSleep();
      return 'Screen can now dim and sleep.';
    },
  },
  {
    id: 'is-kept-awake',
    label: 'Check if kept awake',
    description: 'Checks if the device is currently being kept awake.',
    inputs: [],
    run: async () => {
      const { isKeptAwake } = await plugin.isKeptAwake();
      return { isKeptAwake };
    },
  },
  {
